...
export default function UserInfoUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...userInfoRecord };
    setCurrentBooks(cleanValues.currentBooks);
    setBookHistory(cleanValues.bookHistory);
    setFines(cleanValues.fines);
    setAdmin(cleanValues.admin);
    setSelfID(cleanValues.selfID);
    setErrors({});
  };
  const [userInfoRecord, setUserInfoRecord] = React.useState(userInfo);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(UserInfo, id) : userInfo;
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          const original = await DataStore.query(UserInfo, id);
